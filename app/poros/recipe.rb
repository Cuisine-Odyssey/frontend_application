class Recipe
  ...
  def initialize(meals)
    ...
    i_hash = meals[:meals][0].select { |ing| ing.start_with? 'strIngredient' }.delete_if { |k, v| v.empty? }
    @ingredients = i_hash.values

    a_hash = meals[:meals][0].select { |amt| amt.start_with? 'strMeasure' }.delete_if { |k, v| v.empty? }
